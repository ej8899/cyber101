...
clear
echo "This is our wp-admin access attempt finder."
...
# IP searching in egrep - we'll find 183.*.*.*
...
# IP address "pattern matching" using "sed" and "egrep"
...
# you can replace it with any plain text like we used in the examples above.
IP_PATTERN="208.*.*.*"
...
REGEX_PATTERN=$(echo "$IP_PATTERN" | sed 's/\./\\./g' | sed 's/\*/[0-9]\{1,3\}/g')
